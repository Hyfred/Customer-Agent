...
def main(args, data):
    print(f"=== initialize {args.model_name_or_path} ===")
...
# test_text = '''
# \nOkay, the final answer is the product name from the SQL result.\n<tool_call>\n{ \"name\": \"code_interpreter\", \"arguments\": { \"code\": \"from data_loader import load_trajectory, sql_query\\n\\nconn, df = load_trajectory('8809ec35b5')\\nprint('Data preview:')\\nprint(df.head())\\nprint('Columns:')\\nprint(df.columns)\\n\\n# SQL query to find the product name of the first product clicked\\nsql_query_command = \\\"\\\"\\\"\\nSELECT product_name\\nFROM events\\nWHERE action_type = 'click'\\nORDER BY timestamp ASC\\nLIMIT 1\\n\\\"\\\"\\\"\\n\\nvalidate_true, sql_result = sql_query(conn, sql_query_command)\\nprint('SQL Result:')\\nprint(sql_result)\\n\\n# Final answer\\nfinal_answer = sql_result[0] if sql_result else None\\nprint('\\\\boxed{')\\nprint(final_answer)\\nprint('}')\"}}\n
# '''

# extrac_code=extract_python_code(test_text)
# executor = PythonExecutor()
# results = executor.batch_apply([extrac_code])
# print(results)
